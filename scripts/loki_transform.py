...
from transformations.utility_routines import (
...
from transformations.single_column_claw import (
...
@click.option(
    ...
    show_default=True,
    help=(
        "Enable / disable debug mode. This automatically attaches "
...
@click.option(
    "--cpp/--no-cpp", default=False, help="Trigger C-preprocessing of source files."
)
@click.option(
    "--directive",
    default="openacc",
    type=click.Choice(["openacc", "openmp", "none"]),
...
@click.option(
    "--frontend",
    default="fp",
    type=click.Choice(["fp", "ofp", "omni"]),
    help="Frontend parser to use (default FP)",
)
@click.option(
    "--trim-vector-sections",
    is_flag=True,
    default=False,
...
@click.option(
    "--remove-derived-args/--no-remove-derived-args",
    default=False,
    help="Remove derived-type arguments and replace with canonical arguments",
)
@click.option(
    "--inline-members/--no-inline-members",
    default=False,
    help="Inline member functions for SCC-class transformations.",
)
@click.option(
    "--derive-argument-array-shape/--no-derive-argument-array-shape",
    default=False,
...
def convert(
    ...
    if "scc" in mode:
        scheduler.process(
            RemoveCallsTransformation(
                routines=config.default.get("utility_routines", None)
                or ["DR_HOOK", "ABOR1", "WRITE(NULOUT"],
    ...
    if derive_argument_array_shape:
        ...
        scheduler.process(
            transformation=ExplicitArgumentArrayShapeTransformation(), reverse=True
        )
    ...
    if mode == "scc-hoist":
        ...
        scheduler.process(
            HoistTemporaryArraysAnalysis(dim_vars=(vertical.size,)), reverse=True
        )
    ...
    if mode == "cuf-hoist":
        ...
        scheduler.process(
            transformation=HoistTemporaryArraysAnalysis(dim_vars=(vertical.size,)),
            reverse=True,
        )
        scheduler.process(
...
@click.option("--source", "-s", type=click.Path(), help="Source file to convert.")
@click.option("--driver", "-d", type=click.Path(), help="Driver file to convert.")
@click.option(
    "--cpp/--no-cpp", default=False, help="Trigger C-preprocessing of source files."
)
@click.option(
...
def transpile(build, header, source, driver, cpp, include, define, frontend, xmod):
    ...
    driver[driver_name].apply(
        transformation, role="driver", item=driver_item, successors=(kernel_item,)
    )
    ...
    # Housekeeping: Inject our re-named kernel and auto-wrapped it in a module
    dependency = DependencyTransformation(
        suffix="_FC", mode="module", module_suffix="_MOD"
    )
...
@click.option("--plan-file", type=click.Path(), help='CMake "plan" file to generate.')
def plan(
    mode,
    config,
    header,
    ...
    scheduler = Scheduler(
        paths=paths, config=config, frontend=frontend, full_parse=False, preprocess=cpp
    )

    # Construct the transformation plan as a set of CMake lists of source files
    scheduler.write_cmake_plan(
        filepath=plan_file, mode=mode, buildpath=build, rootpath=root
    )
