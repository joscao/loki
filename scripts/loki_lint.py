...
@click.option(
    ...
    show_default=True,
    help=(
        "Enable / disable debug mode. This incures more verbose "
...
@click.option(
    "--rules-module",
    default="ifs_coding_standards_2011",
    show_default=True,
...
def rules(ctx, with_title, sort_by):  # pylint: disable=unused-argument
    ...
    if with_title:
        ...
        rule_titles = [
            rule.docs.get("title", "").format(**rule.config)
            if rule.config
            else rule.docs.get("title", "")
        ...
        output_string = "\n".join(
            fmt_string.format(
                name=name,
                name_width=max_width_name,
                id=id_,
                id_width=max_width_id,
                title=title,
            )
...
@click.option(
    ...
    multiple=True,
    help=(
        "File name or pattern for file names to be checked. "
...
@click.option(
    ...
    multiple=True,
    help=(
        "File name or pattern for file names to be excluded. "
...
@click.option(
    ...
    help=(
        "Base directory relative to which --include/--exclude "
...
@click.option(
    ...
    type=str,
    help=(
        "When fixing, create a backup of the original file with " "the given suffix."
    ),
)
@click.option(
    "--worker",
    ...
    show_default=True,
    help=(
        "Number of worker processes to use. With --debug enabled "
...
@click.option(
    ...
    is_flag=False,
    flag_value="violations.yml",
    default=None,
    help=(
        "Write a YAML file that lists for every file the violated rules. "
...
def check(
    ...
    if include:
        if "include" in config_values:
            ...
            config_values["include"] = as_tuple(config_values["include"]) + as_tuple(
                include
            )
        else:
    ...
    if exclude:
        if "exclude" in config_values:
            ...
            config_values["exclude"] = as_tuple(config_values["exclude"]) + as_tuple(
                exclude
            )
        else:
    ...
    if basedir:
        if "basedir" in config_values:
            warning(
                "Overwriting `basedir` value in the config file with command line argument"
            )
