...
config.register(
    "print-config",
    False,
    env_variable="LOKI_PRINT_CONFIG",
...
config.register(
    "log-level",
    "INFO",
    env_variable="LOKI_LOGGING",
    callback=set_log_level,
    preprocess=lambda i: log_levels[i],
)

config.register(
    "debug",
    None,
    env_variable="LOKI_DEBUG",
    callback=set_excepthook,
    preprocess=lambda i: auto_post_mortem_debugger if i else None,
)
...
config.register(
    "cpp-dump-files",
    False,
    env_variable="LOKI_CPP_DUMP_FILES",
    preprocess=lambda i: bool(i) if isinstance(i, int) else i,
)
...
config.register(
    "omni-dump-xml",
    False,
    env_variable="LOKI_OMNI_DUMP_XML",
    preprocess=lambda i: bool(i) if isinstance(i, int) else i,
)
...
config.register(
    ...
    False,
    env_variable="LOKI_FRONTEND_STRICT_MODE",
    preprocess=lambda i: bool(i) if isinstance(i, int) else i,
)
...
config.register(
    "case-sensitive",
    False,
    env_variable="LOKI_CASE_SENSITIVE",
    preprocess=lambda i: bool(i) if isinstance(i, int) else i,
)

# Specify a timeout for the REGEX frontend to catch catastrophic backtracking
config.register(
    "regex-frontend-timeout",
    30,
    env_variable="LOKI_REGEX_FRONTEND_TIMEOUT",
    preprocess=int,
)
