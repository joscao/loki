...
class DaceCodegen(PyCodegen):
    ...
    def visit_Subroutine(self, o, **kwargs):
        ...
        arguments = [
            f"{arg.name.lower()}: {self.visit(arg.type, **kwargs)}"
            for arg in o.arguments
            if arg.name.lower() not in symbols
        ]
        header += [
            self.format_line('{name} = dace.symbol("{name}")'.format(name=s))
            for s in symbols
        ]
        header += [self.format_line("@dace.program")]
        header += [
            self.format_line(
                "def ", o.name.lower(), "(", self.join_items(arguments), "):"
            )
        ]
