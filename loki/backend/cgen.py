...
class CCodeMapper(LokiStringifyMapper):
    ...
    def map_cast(self, expr, enclosing_prec, *args, **kwargs):
        ...
        return self.parenthesize_if_needed(
            self.format("(%s) %s", c_intrinsic_type(_type), expression),
            enclosing_prec,
            PREC_CALL,
        )
    ...
    def map_variable_symbol(self, expr, enclosing_prec, *args, **kwargs):
        ...
        if expr.parent is not None:
            parent = self.parenthesize(
                self.rec(expr.parent, PREC_NONE, *args, **kwargs)
            )
...
class CCodegen(Stringifier):
    ...
    def visit_Subroutine(self, o, **kwargs):
        ...
        header = [
            self.format_line("#include <", name, ">") for name in standard_imports
        ]
        ...
        for a in o.arguments:
            ...
            if isinstance(a, Array) > 0:
                ...
                aptr += [""]
        arguments = [
            f"{self.visit(a.type, **kwargs)} {p}{a.name.lower()}"
            for a, p in zip(o.arguments, aptr)
        ]
        header += [
            self.format_line("int ", o.name, "(", self.join_items(arguments), ") {")
        ]
        ...
        body = [
            self.visit(
                o.spec, skip_imports=True, skip_argument_declarations=True, **kwargs
            )
        ]
        ...
        if any(isinstance(a, Array) for a in o.arguments):
            ...
            for a in o.arguments:
                if isinstance(a, Array):
                    ...
                    outer_dims = "".join(
                        f"[{self.visit(d, **kwargs).lower()}]" for d in a.dimensions[1:]
                    )
                    body += [
                        self.format_line(
                            dtype,
    ...
    def visit_VariableDeclaration(self, o, **kwargs):
        ...
        return self.format_line(
            dtype, " ", self.join_items(variables), ";", comment=comment
        )
    ...
    def visit_Loop(self, o, **kwargs):
        ...
        control = "for ({var} = {start}; {var} {crit} {end}; {var} += {incr})".format(
            ...
            crit="<="
            if not o.bounds.step or symbolic_op(o.bounds.step, gt, Literal(0))
            else ">=",
    ...
    def visit_Conditional(self, o, **kwargs):
        ...
        if is_elseif:
            header = self.format_line(
                "} else if (", self.visit(o.condition, **kwargs), ") {"
            )
        else:
