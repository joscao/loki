...
def collect_coefficients(expr):
    ...
    for base, factor in summands.items():
        ...
        if factor == 1 and len(base) == 1:
            ...
            components.append(
                sym.Product(as_tuple(_get_coefficient(factor) + list(base)))
            )
...
class Simplification(enum.Flag):
    ...
    ALL = (
        Flatten | IntegerArithmetic | CollectCoefficients
...
class SimplifyMapper(LokiIdentityMapper):
    ...
    def __init__(
        self, enabled_simplifications=Simplification.ALL, invalidate_source=True
    ):
