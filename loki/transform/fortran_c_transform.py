...
class FortranCTransformation(Transformation):
    ...
    def transform_subroutine(self, routine, **kwargs):
        ...
        for arg in routine.arguments:
            if isinstance(arg.type.dtype, DerivedType):
                self.c_structs[arg.type.dtype.name.lower()] = self.c_struct_typedef(
                    arg.type
                )
    ...
    def c_struct_typedef(cls, derived):
        ...
        """
        typename = (
            f"{derived.name if isinstance(derived, TypeDef) else derived.dtype.name}_c"
        )
        typedef = TypeDef(
            name=typename.lower(), body=(), bind_c=True
    ...
    def iso_c_intrinsic_import(scope):
        symbols = as_tuple(
            Variable(name=name, scope=scope)
            for name in ["c_int", "c_double", "c_float"]
        )
    ...
    def generate_iso_c_wrapper_routine(cls, routine, c_structs, bind_name=None):
        ...
        interface = cls.generate_iso_c_interface(
            routine, bind_name, c_structs, scope=wrapper
        )
        ...
        wrapper_spec.append(
            struct.clone(parent=wrapper) for struct in c_structs.values()
        )
        ...
        for arg in routine.arguments:
            if isinstance(arg.type.dtype, DerivedType):
                ctype = SymbolAttributes(
                    DerivedType(name=c_structs[arg.type.dtype.name.lower()].name)
                )
                cvar = Variable(name=f"{arg.name}_c", type=ctype, scope=wrapper)
                cast_in = InlineCall(
                    ProcedureSymbol("transfer", scope=wrapper),
                    parameters=(arg,),
                    kw_parameters={"mold": cvar},
                )
                ...
                cast_out = InlineCall(
                    ProcedureSymbol("transfer", scope=wrapper),
                    parameters=(cvar,),
                    kw_parameters={"mold": arg},
                )
                casts_out += [Assignment(lhs=arg, rhs=cast_out)]
                local_arg_map[arg.name] = cvar

        arguments = tuple(
            local_arg_map[a] if a in local_arg_map else Variable(name=a)
            for a in routine.argnames
        )
        wrapper_body = casts_in
        wrapper_body += [
            CallStatement(
                name=Variable(name=interface.body[0].name), arguments=arguments
        ...
        wrapper.variables = tuple(
            arg.clone(scope=wrapper) for arg in routine.arguments
        ) + tuple(local_arg_map.values())
    ...
    def generate_iso_c_wrapper_module(cls, module):
        ...
        for decl in FindNodes(VariableDeclaration).visit(module.spec):
            for v in decl.symbols:
                if (
                    isinstance(v.type.dtype, DerivedType)
                    or v.type.pointer
                ...
                getter.spec = Section(
                    body=(Import(module=module.name, symbols=(v.clone(scope=getter),)),)
                )
                isoctype = SymbolAttributes(
                    v.type.dtype, kind=cls.iso_c_intrinsic_kind(v.type, getter)
                )
                if isoctype.kind in ["c_int", "c_float", "c_double"]:
                    getter.spec.append(
                        Import(module="iso_c_binding", symbols=(isoctype.kind,))
                    )
                getter.body = Section(
                    body=(
                        Assignment(lhs=Variable(name=gettername, scope=getter), rhs=v),
                    )
                )
                getter.variables = as_tuple(
                    Variable(name=gettername, type=isoctype, scope=getter)
                )
                wrappers += [getter]
        ...
        for fct in module.subroutines:
            if fct.is_function:
                ...
                for arg in intf_fct.arguments:
                    ...
                    var = Variable(
                        name=arg.name, dimensions=dimensions, type=ctype, scope=intf_fct
                    )
                    intf_args += (var,)
    ...
    def generate_iso_c_interface(cls, routine, bind_name, c_structs, scope):
        ...
        intf_routine = Subroutine(
            name=intf_name, body=None, args=(), parent=scope, bind=bind_name
        )
        ...
        for arg in routine.arguments:
            if isinstance(arg.type.dtype, DerivedType):
                ...
                ctype = SymbolAttributes(
                    DerivedType(name=struct_name), shape=arg.type.shape
                )
            else:
                # Only scalar, intent(in) arguments are pass by value
                # Pass by reference for array types
            ...
            dimensions = arg.dimensions if isinstance(arg, Array) else None
            var = Variable(
                name=arg.name, dimensions=dimensions, type=ctype, scope=intf_routine
            )
    ...
    def generate_c_header(self, module, **kwargs):
        ...
        for td in FindNodes(TypeDef).visit(module.spec):
            header_td = TypeDef(
                name=td.name.lower(), body=(), parent=header_module
            )  # pylint: disable=unexpected-keyword-arg
            declarations = []
            for decl in td.declarations:
                ...
                for v in decl.symbols:
                    ...
                    if isinstance(v, Array):
                        new_shape = as_tuple(
                            d for d in v.shape if not isinstance(d, RangeIndex)
                        )
                        new_type = v.type.clone(shape=new_shape)
                        variables += [
                            v.clone(name=v.name.lower(), type=new_type, scope=header_td)
                        ]
                    else:
                        variables += [v.clone(name=v.name.lower(), scope=header_td)]
                declarations += [
                    VariableDeclaration(
                        symbols=as_tuple(variables),
        ...
        for fct in module.subroutines:
            if fct.is_function:
                ...
                if fct.name in fct.variables:
                    fct_type = self.c_intrinsic_kind(
                        fct.variable_map[fct.name.lower()].type, header_module
                    )

                args = [
                    f"{self.c_intrinsic_kind(a.type, header_module)} {a.name.lower()}"
    ...
    def generate_c_kernel(self, routine, **kwargs):
        ...
        module_variables = {
            im.module.lower(): [
                s.clone(scope=kernel, type=s.type.clone(imported=None, module=None))
                for s in im.symbols
                if isinstance(s, Scalar)
                and s.type.dtype is not BasicType.DEFERRED
                and not s.type.parameter
            ]
        ...
        for module, variables in module_variables.items():
            for var in variables:
                ...
                vget = Assignment(
                    lhs=var, rhs=InlineCall(ProcedureSymbol(getter, scope=var.scope))
                )
                getter_calls += [vget]
        ...
        for im in kernel.imports:
            if str(im.module).upper() in self.__fortran_intrinsic_modules:
                ...
                import_map[im] = im.clone(
                    module=f"{im.module.lower()}_c.h", c_import=True, symbols=()
                )
        ...
        intrinsic_map = {
            i: None
            for i in FindNodes(Intrinsic).visit(kernel.spec)
        ...
        for arg in kernel.arguments:
            if not (arg.type.intent.lower() == "in" and isinstance(arg, Scalar)):
                ...
                if isinstance(arg.type.dtype, DerivedType):
                    ...
                    typedef = _type.dtype.typedef.clone(
                        name=_type.dtype.typedef.name.lower()
                    )
