...
def region_hoist(routine):
        ...
        for region in FindNodes(PragmaRegion).visit(routine.body):
            if is_loki_pragma(region.pragma, starts_with="region-hoist"):
                parameters = get_pragma_parameters(
                    region.pragma, starts_with="region-hoist"
                )
        ...
        for start, stop in regions:
            ...
            if collapse > 0:
                ...
                if len(scopes) <= collapse:
                    raise RuntimeError(
                        f"Not enough enclosing scopes for collapse({collapse})"
                    )
                scopes = scopes[-(collapse + 1) :]
                region = NestedMaskedTransformer(
                    start=start, stop=stop, mapper={start: None}
                ).visit(scopes[0])
                ...
                promote_vars = [
                    var.strip().lower()
                    for var in get_pragma_parameters(start)
                    .get("promote", "")
                    .split(",")
                    if var
                ]
    ...
    routine.body = MaskedTransformer(
        active=True, start=starts, stop=stops, mapper=hoist_map
    ).visit(routine.body)
    num_targets = sum(
        1 for pragma in hoist_map if "target" in get_pragma_parameters(pragma)
    )
    info(
        "%s: hoisted %d region(s) in %d group(s)",
        routine.name,
        len(hoist_map) - num_targets,
        num_targets,
    )
...
def region_to_call(routine):
    ...
    imports = {
        var for imprt in FindNodes(Import).visit(routine.spec) for var in imprt.symbols
    }
    mask_map = {}
    with pragma_regions_attached(routine):
        with dataflow_analysis_attached(routine):
            for region in FindNodes(PragmaRegion).visit(routine.body):
                ...
                parameters = get_pragma_parameters(
                    region.pragma, starts_with="region-to-call"
                )
                name = parameters.get(
                    "name", f"{routine.name}_region_to_call_{counter}"
                )
                counter += 1

                # Create the external subroutine containing the routine's imports and the region's body
                spec = Section(
                    body=Transformer().visit(FindNodes(Import).visit(routine.spec))
                )
                ...
                region_inout_args = (
                    region.uses_symbols & region.defines_symbols - imports
                )
                region_out_args = region.defines_symbols - region.uses_symbols - imports

                # Remove any parameters from in args
                region_in_args = {
                    arg for arg in region_in_args if not arg.type.parameter
                }
                ...
                region_var_map = CaseInsensitiveDict(
                    ...
                    if v.clone(dimensions=None) not in imports
                )
                pragma_in_args = {
                    region_var_map[v.lower()]
                    for v in parameters.get("in", "").split(",")
                    if v
                }
                pragma_inout_args = {
                    region_var_map[v.lower()]
                    for v in parameters.get("inout", "").split(",")
                    if v
                }
                pragma_out_args = {
                    region_var_map[v.lower()]
                    for v in parameters.get("out", "").split(",")
                    if v
                }

                # Override arguments according to pragma annotations
                region_in_args = (
                    region_in_args - (pragma_inout_args | pragma_out_args)
                ) | pragma_in_args
                region_inout_args = (
                    region_inout_args - (pragma_in_args | pragma_out_args)
                ) | pragma_inout_args
                region_out_args = (
                    region_out_args - (pragma_in_args | pragma_inout_args)
                ) | pragma_out_args
                ...
                for intent, args in zip(
                    ("in", "inout", "out"),
                    (region_in_args, region_inout_args, region_out_args),
                ):
                    for arg in args:
                        local_var = region_routine_var_map[arg.name]
                        local_var = local_var.clone(
                            type=local_var.type.clone(intent=intent)
                        )
    ...
    routine.body = MaskedTransformer(
        active=True, start=starts, stop=stops, mapper=mask_map
    ).visit(routine.body)
