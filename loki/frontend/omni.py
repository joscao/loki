...
from loki.tools import as_tuple, execute, gettempdir, filehash, CaseInsensitiveDict
from loki.pragma_utils import process_dimension_pragmas, pragmas_attached
...
@Timer(
    logger=debug, text=lambda s: f"[Loki::OMNI] Executed parse_omni_file in {s:.2f}s"
)
...
@Timer(
    logger=debug, text=lambda s: f"[Loki::OMNI] Executed parse_omni_source in {s:.2f}s"
)
...
def parse_omni_ast(
    ast, definitions=None, type_map=None, symbol_map=None, raw_source=None, scope=None
):
    """
...
class OMNI2IR(GenericVisitor):
    ...
    def visit_FinterfaceDecl(self, o, **kwargs):
        ...
        if name is not None:
            ...
            if name not in scope.symbol_attrs:
                scope.symbol_attrs[name] = SymbolAttributes(
                    ProcedureType(name, is_generic=True)
                )
        ...
        return ir.Interface(
            body=body, abstract=abstract, spec=spec, source=kwargs["source"]
        )
    ...
    def visit_FfunctionDefinition(self, o, **kwargs):
        ...
        # Instantiate the object
        routine = self._create_Subroutine_object(
            o, kwargs["scope"], kwargs["symbol_map"]
        )
        if routine is None:
            return None
            ...
            spec_map.update(
                {
                    d: None
                    for d in FindNodes(
                        (ir.ProcedureDeclaration, ir.VariableDeclaration)
                    ).visit(spec)
        ...
        if not f_imports:
            ...
            spec.insert(
                spec.body.index(f_imports[-1]) + 1, ir.Intrinsic(text="IMPLICIT NONE")
            )
    ...
    def visit_FmoduleProcedureDecl(self, o, **kwargs):
        ...
        return ir.ProcedureDeclaration(
            symbols=symbols, module=True, source=kwargs.get("source")
        )
    ...
    def visit_FmoduleDefinition(self, o, **kwargs):
        ...
        if contains_ast is not None:
            ...
            contains = [
                self._create_Subroutine_object(
                    member_ast, kwargs["scope"], kwargs["symbol_map"]
                )
    ...
    def visit_varDecl(self, o, **kwargs):
        ...
        if name.attrib["type"] in self._omni_types:
            ...
            dimensions = as_tuple(
                self.visit(d, **kwargs) for d in tast.findall("indexRange")
            )
            if dimensions:
                _type = _type.clone(shape=dimensions)
            ...
            if isinstance(_type.dtype, ProcedureType):
                if _type.dtype.name == "UNKNOWN":
                    ...
                    dtype = ProcedureType(
                        variable.name,
                        is_function=_type.dtype.is_function,
                        return_type=_type.dtype.return_type,
                    )
                ...
                if tast.attrib.get("is_external") == "true":
                    ...
                    variable == kwargs["scope"].name
                    and _type.dtype.return_type is not None
                ):
        ...
        if o.find("value") is not None:
            _type = _type.clone(
                initial=AttachScopesMapper()(
                    self.visit(o.find("value"), **kwargs), scope=scope
                )
            )
        ...
        if isinstance(_type.dtype, ProcedureType):
            ...
            if _type.external:
                return ir.ProcedureDeclaration(
                    symbols=(variable,), external=True, source=kwargs["source"]
                )
            if _type.dtype.name == variable and _type.dtype.is_function:
                return ir.ProcedureDeclaration(
                    symbols=(variable,),
                    interface=_type.dtype.return_type.dtype,
                    source=kwargs["source"],
                )
            interface = sym.Variable(
                name=_type.dtype.name, scope=scope.get_symbol_scope(_type.dtype.name)
            )
            return ir.ProcedureDeclaration(
                symbols=(variable,), interface=interface, source=kwargs["source"]
            )
            ...
            for v in variables:
                if isinstance(v.type.dtype, ProcedureType):
                    if v.type.dtype.name == v and v.type.dtype.is_function:
                        ...
                        interface = sym.Variable(
                            name=iface_name,
                            scope=kwargs["scope"].get_symbol_scope(iface_name),
                        )
        ...
        if o.find("binding"):
            ...
            if bind_name_scope is not None:
                ...
                bind_name = bind_name.clone(
                    type=bind_name.type.clone(dtype=ProcedureType(bind_name.name))
                )
    ...
    def visit_FdataDecl(self, o, **kwargs):
        ...
        return ir.DataDeclaration(
            variable=variable, values=values, source=kwargs["source"]
        )
    ...
    def visit_FbasicType(self, o, **kwargs):
        ...
        if ref in self._omni_types:
            ...
            kind = (
                self.visit(o.find("kind"), **kwargs)
                if o.find("kind") is not None
                else None
            )
            length = o.find("len")
            ...
            if o.find("name") is not None:
                _type = self.visit(
                    self.type_map[ref], name=o.find("name").text, **kwargs
                )
            else:
    ...
    def visit_FfunctionType(self, o, **kwargs):
        if o.attrib["return_type"] == "Fvoid":
            ...
            return_type = SymbolAttributes(
                BasicType.from_fortran_type(self._omni_types[o.attrib["return_type"]])
            )
        ...
        dtype = ProcedureType(
            name, is_function=return_type is not None, return_type=return_type
        )
    ...
    def visit_associateStatement(self, o, **kwargs):
        ...
        associate = ir.Associate(
            associations=(), body=(), parent=parent_scope, source=kwargs["source"]
        )
    ...
    def visit_FdoStatement(self, o, **kwargs):
        ...
        return ir.Loop(
            variable=variable, body=body, bounds=bounds, source=kwargs["source"]
        )
    ...
    def visit_FifStatement(self, o, **kwargs):
        ...
        return ir.Conditional(
            condition=condition, body=body, else_body=else_body, source=kwargs["source"]
        )
    ...
    def visit_FselectCaseStatement(self, o, **kwargs):
        ...
        value_idx, case_idx = list(o).index(o.find("value")), list(o).index(
            o.find("FcaseLabel")
        )
    ...
    def visit_FcaseLabel(self, o, **kwargs):
        values = [
            ...
            for value in list(o)
            if value.tag in ("value", "indexRange")
        ]
        if not values:
            values = None
        elif len(values) == 1:
    ...
    def visit_FintConstant(self, o, **kwargs):
        if "kind" in o.attrib:
            _type = self.visit(self.type_map[o.attrib.get("type")], **kwargs)
            return sym.Literal(
                value=int(o.text), type=BasicType.INTEGER, kind=_type.kind
            )
    ...
    def visit_functionCall(self, o, **kwargs):
        ...
        if o.attrib.get("type", "Fvoid") == "Fvoid":
            ...
            return ir.CallStatement(
                name=name, arguments=args, kwarguments=kw_args, source=kwargs["source"]
            )
    ...
    def visit_statementLabel(self, o, **kwargs):
        return ir.Comment(
            "__STATEMENT_LABEL__", label=o.attrib["label_name"], source=kwargs["source"]
        )
