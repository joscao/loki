...
@Timer(
    logger=debug, text=lambda s: f"[Loki::OFP] Executed parse_ofp_source in {s:.2f}s"
)
...
def parse_ofp_ast(ast, pp_info=None, raw_source=None, definitions=None, scope=None):
    ...
    _ir = OFP2IR(
        definitions=definitions, raw_source=raw_source, pp_info=pp_info, scope=scope
    ).visit(ast)
...
class OFP2IR(GenericVisitor):
    ...
    def visit_if(self, o, **kwargs):
        ...
        for stmt in reversed(o.findall("else-if-stmt")):
            ...
            lstart, cstart = int(stmt.attrib["line_begin"]), int(
                stmt.attrib["col_begin"]
            )
            sources += [
                extract_source_from_range(
                    (lstart, lend), (cstart, cend), self._raw_source, label=labels[-1]
                )
            ]
    ...
    def visit_select(self, o, **kwargs):
        ...
        case_stmts, case_stmt_index = zip(
            *[(c, i) for i, c in enumerate(body) if c.tag == "case"]
        )
        ...
        for case_idx, stmt_idx in enumerate(case_stmt_index[1:]):
            ...
            bodies[case_idx - 1] += as_tuple(
                self.visit(c, **kwargs) for c in body[start_idx:stmt_idx]
            )
    ...
    def visit_case(self, o, **kwargs):
        ...
        if isinstance(value, tuple) and len(value) > int(
            o.find("header/value-ranges").attrib["count"]
        ):
    ...
    def visit_comment(self, o, **kwargs):
        ...
        if match_pragma:
            ...
            return ir.Pragma(
                keyword=gd["keyword"], content=gd["content"], source=kwargs["source"]
            )
        return ir.Comment(
            text=o.attrib["text"], label=kwargs["label"], source=kwargs["source"]
        )
    ...
    def visit_statement(self, o, **kwargs):
        ...
        if (
            o.find("where-construct-stmt") is not None
            or o.find("where-stmt") is not None
        ):
            ...
            while True:
                ...
                if construct_start < statement_start:
                    ...
                    while "MASKED_ELSEWHERE_CONSTRUCT" in w_children:
                        ...
                        bodies += [
                            as_tuple(w_children[: iw - 1])
                        ]  # that's the body from the previous case
                        conditions += [
                            w_children[iw - 1]
                        ]  # that's the condition for the next case
                    ...
                    stmts += [
                        ir.MaskedStatement(
                            conditions=as_tuple(conditions),
                            bodies=as_tuple(bodies),
                            default=default,
                            label=kwargs["label"],
                            source=kwargs["source"],
    ...
    def visit_assignment(self, o, **kwargs):
        ...
        return ir.Assignment(
            lhs=lhs, rhs=rhs, label=kwargs["label"], source=kwargs["source"]
        )
    ...
    def visit_pointer_assignment(self, o, **kwargs):
        ...
        rhs = self.visit(o.find("value"), **kwargs)
        return ir.Assignment(
            lhs=lhs, rhs=rhs, ptr=True, label=kwargs["label"], source=kwargs["source"]
        )
        ...
        if interface is not None:
            ...
            if interface_scope is not None:
                ...
                interface = sym.Variable(
                    name=interface, type=SymbolAttributes(ProcedureType(interface))
                )
            ...
            bind_name = sym.Variable(
                name=o.attrib["procedureName"], scope=procedure_scope
            )
    ...
    def visit_generic_binding(self, o, **kwargs):
        ...
        _type = SymbolAttributes(
            ProcedureType(var.name, is_generic=True), bind_names=as_tuple(bind_names)
        )
    ...
    def visit_private_components_stmt(self, o, **kwargs):
        return ir.Intrinsic(
            o.attrib["privateKeyword"], source=kwargs["source"], label=kwargs["label"]
        )

    def visit_sequence_stmt(self, o, **kwargs):
        return ir.Intrinsic(
            o.attrib["sequenceKeyword"], source=kwargs["source"], label=kwargs["label"]
        )
    ...
    def visit_declaration(self, o, **kwargs):
        ...
        if not "type" in o.attrib:
            ...
            if o.find("save-stmt") is not None:
                return ir.Intrinsic(
                    text=source.string.strip(), label=label, source=source
                )
        ...
        if o.attrib["type"] == "external":
            ...
            for var in variables:
                ...
                if _type is None:
                    _type = SymbolAttributes(
                        dtype=ProcedureType(var.name, is_function=False), external=True
                    )
                else:
        ...
        if o.find("derived-type-stmt") is not None:
            ...
            if type_stmt.attrib["hasTypeAttrSpecList"] == "true":
                attrs = dict(
                    self.visit(attr, **kwargs) for attr in o.findall("type-attr-spec")
                )
                ...
                private = (
                    access_spec is not None
                    and access_spec.attrib["keyword"].lower() == "private"
                )
                public = (
                    access_spec is not None
                    and access_spec.attrib["keyword"].lower() == "public"
                )
            else:
                abstract = False
                extends = None
            ...
            for c in o[:contains_idx]:
                ...
                # Procedure declarations
                    ...
                    "comment",
                ):  # Add here other node types. Unfortunately we can't allow all
            ...
            if contains_stmt is not None:
                ...
                for i in o[start_idx:]:
                    ...
                    if i.tag == "specific-binding":
                        ...
                        body.append(
                            self.visit(
                                i, attrs=attrs, names=names, spec=generic_spec, **kwargs
                            )
                        )
                        names = []
                        attrs = {}
        ...
        if o.find("procedures") is not None:
            ...
            return ir.ProcedureDeclaration(
                symbols=symbols, interface=interface, source=source, label=label
            )
    ...
    def visit_interface(self, o, **kwargs):
        ...
        body = []
        grouped_elems = match_tag_sequence(
            o.find("body/specification/declaration"),
            [
    ...
    def _create_Subroutine_object(self, o, scope):
        ...
        if o.tag == "function":
            ...
            args = [
                a.attrib["name"].upper()
                for a in header_ast.findall("arguments/argument")
            ]
        ...
        prefix = [
            a.attrib["spec"].upper() for a in header_ast.findall("t-prefix-spec")
        ] or None
    ...
    def visit_subroutine(self, o, **kwargs):
        ...
        docstring = sanitize_ir(
            docstring, OFP, pp_registry=sanitize_registry[OFP], pp_info=self.pp_info
        )
        spec = self.visit(spec_ast, **kwargs)
        spec = sanitize_ir(
            spec, OFP, pp_registry=sanitize_registry[OFP], pp_info=self.pp_info
        )
        ...
        if not body_ast:
            ...
            body = sanitize_ir(
                body, OFP, pp_registry=sanitize_registry[OFP], pp_info=self.pp_info
            )
    ...
    def visit_associate(self, o, **kwargs):
        ...
        associations = [
            (self.visit(a[0], **kwargs), self.visit(a.find("association"), **kwargs))
    ...
    def visit_allocate(self, o, **kwargs):
        variables = as_tuple(
            self.visit(v, **kwargs) for v in o.findall("expressions/expression/name")
        )
        kw_args = {
            arg.attrib["name"].lower(): self.visit(arg, **kwargs)
        ...
        return ir.Allocation(
            ...
            label=kwargs["label"],
            source=kwargs["source"],
            data_source=kw_args.get("source"),
    ...
    def visit_use(self, o, **kwargs):
        ...
        if o.find("only") is not None:
            ...
            # Rename list
            rename_list = dict(
                self.visit(s, **kwargs) for s in o.findall("rename/rename")
            )
    ...
    def create_intrinsic_from_source(self, o, attrib_name, **kwargs):
        ...
        assert cstart != -1
        return ir.Intrinsic(
            text=source.string[cstart:].strip(), label=label, source=source
        )
    ...
    def visit_cycle_stmt(self, o, **kwargs):
        return self.create_intrinsic_from_source(
            o, "cycleKeyword", label=kwargs["label"], source=kwargs["source"]
        )

    def visit_format_stmt(self, o, **kwargs):
        return self.create_intrinsic_from_source(
            o, "formatKeyword", label=kwargs["label"], source=kwargs["source"]
        )

    def visit_print_stmt(self, o, **kwargs):
        return self.create_intrinsic_from_source(
            o, "printKeyword", label=kwargs["label"], source=kwargs["source"]
        )

    def visit_open_stmt(self, o, **kwargs):
        return self.create_intrinsic_from_source(
            o, "openKeyword", label=kwargs["label"], source=kwargs["source"]
        )

    def visit_close_stmt(self, o, **kwargs):
        return self.create_intrinsic_from_source(
            o, "closeKeyword", label=kwargs["label"], source=kwargs["source"]
        )

    def visit_write_stmt(self, o, **kwargs):
        return self.create_intrinsic_from_source(
            o, "writeKeyword", label=kwargs["label"], source=kwargs["source"]
        )

    def visit_read_stmt(self, o, **kwargs):
        return self.create_intrinsic_from_source(
            o, "readKeyword", label=kwargs["label"], source=kwargs["source"]
        )
    ...
    def visit_label(self, o, **kwargs):
        ...
        return ir.Comment(
            "__STATEMENT_LABEL__", label=kwargs["label"], source=kwargs["source"]
        )
    ...
    def visit_subscripts(self, o, **kwargs):
        return tuple(
            self.visit(c, **kwargs) for c in o if c.tag in ("subscript", "argument")
        )
        ...
        for i, part_ref in enumerate(o.findall("part-ref")):
            ...
            if part_ref.attrib["hasSectionSubscriptList"] == "true":
                if i < num_part_ref - 1 or o.attrib["type"] == "variable":
                    if subscripts[
                        0
                    ]:  # If there are no subscripts it cannot be an array but must
                        # be a function call
                        arguments = subscripts.pop(0)
                        kwarguments = tuple(
                            arg for arg in arguments if isinstance(arg, tuple)
                        )
        ...
        if subscripts:
            ...
            if str(name).upper() in intrinsic_calls or kwarguments:
                return sym.InlineCall(
                    name, parameters=arguments, kw_parameters=kwarguments
                )
    ...
    def visit_literal(self, o, **kwargs):
        ...
        if kind_param is not None:
            ...
            if kind.isnumeric():
                ...
                kw_args["kind"] = AttachScopesMapper()(
        ...
        for op in ops:
            ...
            if op == "+":
                ...
                if len(exprs) > 0:
                    # Binary minus
                    expression = sym.Sum(
                        (expression, sym.Product((-1, exprs.popleft())))
                    )
                else:
                    # Unary minus
                ...
                expression = sym.Quotient(
                    numerator=expression, denominator=exprs.popleft()
                )
            elif op == "**":
                ...
                expression = sym.LogicalOr(
                    (sym.LogicalAnd(e), sym.LogicalNot(sym.LogicalOr(e)))
                )
            elif op == ".neqv.":
                e = (expression, exprs.popleft())
                expression = sym.LogicalAnd(
                    (sym.LogicalNot(sym.LogicalAnd(e)), sym.LogicalOr(e))
                )
            elif op == "//":
    ...
    def create_typedef_procedure_declaration(
        self, comps, iface=None, attrs=None, scope=None, source=None
    ):
        """
    ...
    def create_typedef_variable_declaration(
        self, t, comps, attr=None, scope=None, source=None
    ):
        """
        Utility method to create individual declarations from a group of AST nodes.
        """
        ...
        for v in comps.findall("component"):
            ...
            if deferred_shape is not None:
                ...
                dimensions = [
                    sym.RangeIndex((None, None, None)) for _ in range(dim_count)
                ]
            else:
                dimensions = as_tuple(self.visit(c, scope=scope) for c in v_children)
            dimensions = (
                as_tuple(
                    AttachScopesMapper()(d, scope=scope)
                    for d in dimensions
                    if d is not None
                )
                or None
    ...
    def create_interface_declaration(self, names, proc_stmt, scope=None, source=None):
        ...
        return ir.ProcedureDeclaration(
            symbols=symbols, module=module_proc, source=source
        )
