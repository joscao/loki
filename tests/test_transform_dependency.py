...
def test_dependency_transformation_globalvar_imports(
    frontend, use_scheduler, tempdir, config
):
    """
...
END SUBROUTINE driver
    ...
    if use_scheduler:
        ...
        driver["driver"].apply(
            transformation, role="driver", targets=("kernel", "some_const")
        )
...
@pytest.mark.parametrize(
    "frontend",
    available_frontends(xfail=[(OMNI, "C-imports need pre-processing for OMNI")]),
)
...
""",
    ...
    transformation = DependencyTransformation(
        suffix="_test", mode="strict", include_path=here
    )
...
@pytest.mark.parametrize("use_scheduler", [False, True])
def test_dependency_transformation_module_wrap(
    frontend, use_scheduler, tempdir, config
):
    """
    Test injection of suffixed kernels into unchanged driver
...
@pytest.mark.parametrize("frontend", available_frontends())
@pytest.mark.parametrize("use_scheduler", [False, True])
def test_dependency_transformation_replace_interface(
    frontend, use_scheduler, tempdir, config
):
    """
...
@pytest.mark.parametrize("frontend", available_frontends())
@pytest.mark.parametrize("use_scheduler", [False, True])
def test_dependency_transformation_contained_member(
    frontend, use_scheduler, tempdir, config
):
    """
...
END MODULE header_mod
    ...
    assert len(imports) == 2
    assert (
        "header_var" in imports and imports["header_var"].module.lower() == "header_mod"
    )
    assert (
        "kernel_test" in imports
        and imports["kernel_test"].module.lower() == "kernel_test_mod"
    )
