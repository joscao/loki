...
def compile_and_test(scheduler, here, a=5, b=1, test_name=""):
    ...
    for item in scheduler.items:
        if "driver" in item.name:
            ...
            module = jit_compile(
                item.source,
                filepath=item.source.path.with_suffix(suffix).name,
                objname=None,
            )
...
def check_arguments_and_parameter(
    scheduler, subroutine_arguments, call_arguments, parameter_variables
):
    """
...
def test_parametrise_source(here, frontend, config):
    ...
    compile_and_test(
        scheduler=scheduler, here=here, a=a, b=b, test_name="original_source"
    )
...
def test_parametrise_simple_replace_by_value(here, frontend, config):
    ...
    if frontend == OMNI:
        ...
        routine_spec_str = fgen(
            scheduler.item_map["parametrise#another_driver"].routine.spec
        )
        assert f"c(1:{a}, 1:{b})" in routine_spec_str
...
def test_parametrise_modified_callback(here, frontend, config):
    ...
    def stop_execution(**kwargs):
        ...
        abort = (
            ir.CallStatement(
                name=sym.Variable(name="stop_execution"),
                arguments=(sym.StringLiteral(f"{msg}"),),
            ),
        )
        return abort
    ...
    for i, abort_callback in enumerate(abort_callbacks):
        ...
        transformation = ParametriseTransformation(
            dic2p=dic2p, abort_callback=abort_callback
        )
        scheduler.process(transformation=transformation)
        ...
        compile_and_test(
            scheduler=scheduler, here=here, a=a, b=b, test_name=f"callback_{i+1}"
        )
...
def test_parametrise_non_driver_entry_points(here, frontend, config):
    ...
    transformation = ParametriseTransformation(
        dic2p=dic2p, entry_points=("kernel1", "kernel2")
    )
    scheduler.process(transformation=transformation)
    ...
    compile_and_test(
        scheduler=scheduler, here=here, a=a, b=b, test_name="parametrised_entry_points"
    )
