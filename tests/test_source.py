...
def test_source_find(here):
    ...
    routine_b_match = re.search(
        r"(subroutine routine_b.*?end subroutine routine_b)", fcode, re.DOTALL
    )
    ...
    assert (cstart, cend) == (
        bstart - 1,
        bend,
    )  # start offset by 1 because leading whitespace is taken into account
...
def test_source_clone_with_string(here):
    ...
    source = Source(
        (1, routine_b_fcode.count("\n") + 1), routine_b_fcode
    ).clone_with_string(routine_b_fcode)
    assert source.string == routine_b_fcode
    assert source.lines == (1, routine_b_fcode.count("\n") + 1)
    assert source.file is None

    source = Source(lines, fcode, filepath).clone_with_string(
        routine_b_fcode.upper(), ignore_case=True
    )
    assert source.string == routine_b_fcode
    assert source.lines == routine_b_lines
    assert source.file == filepath
...
def test_source_clone_with_span(here):
    ...
    source = Source(lines, fcode.upper(), filepath).clone_with_span(
        routine_b_match.span()
    )
...
def test_source_clone_lines(here):
    ...
    routine_b_str_lines = str_lines[
        routine_b_source.lines[0] - 1 : routine_b_source.lines[1]
    ]
    assert len(source_lines) == len(routine_b_str_lines)

    for idx, (source_line, str_line) in enumerate(
        zip(source_lines, routine_b_str_lines)
    ):
        assert source_line.string == str_line
...
contains
    ...
    assert all(
        l1.lines[0] <= l1.lines[1]
        and l1.lines[1] + 1 == l2.lines[0]
        and l2.lines[0] <= l2.lines[1]
...
@pytest.mark.parametrize(
    "source_list,expected",
    (
        ([], None),
        (
            [
                Source([1, 2], "subroutine my_routine\nimplicit none"),
                Source([3, None], "end subroutine my_routine"),
            ],
            Source(
                [1, 3],
                "subroutine my_routine\nimplicit none\nend subroutine my_routine",
...
subroutine my_routine
  ...
  var_2 = 2._real64
        ...
        (
            [Source([1, 1], 'print *,* "hello world!"')],
            Source([1, 1], 'print *,* "hello world!"'),
        ),
        (
            [
                Source([13, 19], "! line with less line breaks than reported"),
                Source([20, None], "! here"),
            ],
...
def test_fortran_reader(here):
    ...
    assert all(
        ...
        for line in source.string.splitlines()
    )
    ...
    end = new_reader.sanitized_string.find("end subroutine foo_sub") + len(
    ...
    end = reader.sanitized_string.find("end function func_simple") + len(
    ...
    end = reader.sanitized_string.find("end subroutine sub_with_end") + len(
