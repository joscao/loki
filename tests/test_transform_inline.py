...
"""
    ...
    v2, v3 = reference.transform_inline_elemental_functions(11.0)
    assert v2 == 66.0
    assert v3 == 666.0
    ...
    v2, v3 = kernel.transform_inline_elemental_functions_(11.0)
    assert v2 == 66.0
    assert v3 == 666.0
...
"""
    ...
    reference = jit_compile_lib(
        [module, param_module], path=here, name=refname, builder=builder
    )

    (
        v2,
        v3,
    ) = reference.transform_inline_constant_parameters_mod.transform_inline_constant_parameters(
        10
    )
    assert v2 == 8
    assert v3 == 2
    ...
    assert (
        len(
            FindNodes(Import).visit(module["transform_inline_constant_parameters"].spec)
        )
        == 1
    )
    for routine in module.subroutines:
        inline_constant_parameters(routine, external_only=True)
    assert not FindNodes(Import).visit(
        module["transform_inline_constant_parameters"].spec
    )

    # Hack: rename module to use a different filename in the build
    module.name = f"{module.name}_"
    obj = jit_compile_lib(
        [module], path=here, name=f"{module.name}_{frontend}", builder=builder
    )

    (
        v2,
        v3,
    ) = obj.transform_inline_constant_parameters_mod_.transform_inline_constant_parameters(
        10
    )
    assert v2 == 8
    assert v3 == 2
...
"""
    ...
    v1 = (
        reference.transform_inline_constant_parameters_kind_mod.transform_inline_constant_parameters_kind()
    )
    assert v1 == 5.0
    (here / f"{module.name}.f90").unlink()
    (here / f"{param_module.name}.f90").unlink()
    ...
    assert (
        len(
            FindNodes(Import).visit(
                module["transform_inline_constant_parameters_kind"].spec
            )
        )
        == 1
    )
    ...
    v1 = (
        obj.transform_inline_constant_parameters_kind_mod_.transform_inline_constant_parameters_kind()
    )
    assert v1 == 5.0
...
subroutine member_routines(a, b)
  ...
  contains
    ...
    b = np.array([3.0, 3.0, 3.0], order="F")
    reference(a, b)

    assert (a == [6.0, 7.0, 8.0]).all()
    assert (b == [3.0, 3.0, 3.0]).all()
...
@pytest.mark.parametrize(
    "frontend", available_frontends(xfail=[(OMNI, "No header information in test")])
)
