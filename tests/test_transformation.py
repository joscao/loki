...
def fixture_rename_transform():
    class RenameTransform(Transformation):
        ...
        def transform_file(self, sourcefile, **kwargs):
            sourcefile.ir.prepend(Comment(text="! [Loki] RenameTransform applied"))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "myroutine",
            lambda transform, obj, **kwargs: transform.apply_subroutine(obj, **kwargs),
        ),
    ],
)
@pytest.mark.parametrize("lazy", [False, True])
def test_transformation_apply_subroutine(
    rename_transform, frontend, target, apply_method, lazy
):
    """
...
"""
    ...
    assert (
        source._incomplete is lazy
    )  # This should only have triggered a re-parse on the actual transformation target
...
def test_transformation_post_apply_subroutine(
    here, frontend, post_apply_rescope_symbols
):
...
subroutine transformation_post_apply(i)
  ...
  i = 1
    ...
    routine.apply(
        ScopingErrorTransformation(),
        post_apply_rescope_symbols=post_apply_rescope_symbols,
    )
...
contains
  subroutine test_post_apply(ret)
    ...
    new_filepath = here / (
        f"{module.name}_{frontend}_{post_apply_rescope_symbols!s}.f90"
    )
...
"""
    ...
    if ricks_path.exists():
        ricks_path.unlink()
    FileWriteTransformation(builddir=here, mode="roll", suffix=".java").apply(
