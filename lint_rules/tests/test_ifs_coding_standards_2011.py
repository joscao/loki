...
pytestmark = pytest.mark.skipif(
    not available_frontends(), reason="Suitable frontend not available"
)
...
integer baz
    ...
    assert all(
        keyword in messages[0] for keyword in ("module_naming_mod.f90", "filename")
    )
    assert all(keyword in messages[1] for keyword in ('"_mod"', "Name of module"))
    assert all(
        keyword in messages[2] for keyword in ("module_naming_mod.f90", "filename")
    )
...
if (lhook) call dr_hook('routine_contained_not_okay', 1, zhook_handle)
    ...
    assert all(
        "First executable statement must be call to DR_HOOK" in messages[i]
        for i in [0, 4, 6]
    )
    assert all(
        "Last executable statement must be call to DR_HOOK" in messages[i]
        for i in [5, 7]
    )
    assert all(
        'String argument to DR_HOOK call should be "' in messages[i] for i in [1, 8]
    )
    ...
    assert all(
        f"routine_not_okay_{letter}" in messages[i]
...
if (lhook) call dr_hook('some_mod:mod_contained_routine_not_okay', 8, zhook_handle)
    ...
    assert all(
        'String argument to DR_HOOK call should be "' in messages[i] for i in [0, 1, 2]
    )
...
@pytest.mark.parametrize("frontend", available_frontends())
@pytest.mark.parametrize(
    "max_num_statements, passes", [(10, True), (4, True), (3, False)]
)
...
associate (aa=>a)
    ...
    handler = DefaultHandler(target=messages.append)
    config = {
        "LimitSubroutineStatementsRule": {"max_num_statements": max_num_statements}
    }
    _ = run_linter(
        source, [rules.LimitSubroutineStatementsRule], config=config, handlers=[handler]
    )
...
@pytest.mark.parametrize("frontend", available_frontends())
@pytest.mark.parametrize(
    "max_num_arguments, passes", [(10, True), (8, True), (7, False), (1, False)]
)
...
print *, a, b, c, d, e, f, g, h
    ...
    keywords = (
        ...
        "[3.6]",
        "8",
        str(max_num_arguments),
        "routine_max_dummy_args",
    )
    assert all(all(keyword in msg for keyword in keywords) for msg in messages)
...
@pytest.mark.parametrize("frontend", available_frontends())
@pytest.mark.parametrize(
    "banned_statements, passes",
    [([], True), (["GO TO"], False), (["GO TO", "RETURN"], False), (["RETURN"], True)],
)
...
100 continue
    ...
    _ = run_linter(
        source, [rules.BannedStatementsRule], config=config, handlers=[handler]
    )
