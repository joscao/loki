...
def get_file_manager(ctx):
    ...
    if not ctx.obj["name"]:
        raise click.UsageError(
            'No site name given. Provide "--name" before the command.'
        )
    ...
    site = Site.from_space_and_name(space=ctx.obj["space"], name=ctx.obj["name"])
...
def upload(ctx, local_path, remote_path, clean):
    ...
    if clean:
        try:
            ...
            pass
    my_site_manager.upload(
        local_path=local_path, remote_path=remote_path, recursive=True
    )
