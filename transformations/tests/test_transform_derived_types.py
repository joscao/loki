...
contains
    ...
    item = SubroutineItem(
        name="transform_derived_type_arguments_mod#kernel", source=source
    )
    source["kernel"].apply(
        DerivedTypeArgumentsTransformation(), role="kernel", item=item
    )
    ...
    for members in item.trafo_data[DerivedTypeArgumentsTransformation._key][
        "expansion_map"
    ].values():
...
contains
    ...
    call_tree = [
        SubroutineItem(
            name="transform_derived_type_arguments_mod#caller",
            source=source,
            config={"role": "driver"},
        ),
        SubroutineItem(
            name="transform_derived_type_arguments_mod#kernel",
            source=source,
            config={"role": "kernel"},
        ),
    ]
    ...
    for item, successor in reversed(list(zip_longest(call_tree, call_tree[1:]))):
        transformation.apply(
            item.routine, role=item.role, item=item, successors=as_tuple(successor)
        )
...
subroutine driver(arr, n, s, t)
    ...
    source_driver = Sourcefile.from_source(
        fcode_driver, frontend=frontend, definitions=source_my_mod.definitions
    )

    kernel = SubroutineItem(
        "my_mod#kernel", config={"role": "kernel"}, source=source_my_mod
    )
    ...
    transformation.apply(
        driver.routine, item=driver, role=driver.role, successors=[kernel]
    )
...
contains
    ...
    for item in call_tree:
        if item.role == "driver":
            ...
            assert (
                item.trafo_data[transformation._key]["orig_argnames"]
                == orig_args[item.routine.name.lower()]
            )
...
subroutine caller(z)
    ...
    bucket = Sourcefile.from_source(
        fcode_bucket, frontend=frontend, definitions=header.definitions
    )
    layer = Sourcefile.from_source(
        fcode_layer,
        frontend=frontend,
        definitions=header.definitions + bucket.definitions,
    )
    ...
    for name, successors in reversed(call_tree):
        ...
        transformation.apply(
            item.routine, role=item.role, item=item, successors=as_tuple(children)
        )
    ...
    assert items["setup"].routine.arguments == ("t_a(:)", "t_b(:, :)", "m", "n")

    # Check arguments of init
    assert items["init"].routine.arguments == ("t_a(:)", "t_b(:, :)", "m", "n")

    # Check arguments of teardown
    assert items["teardown"].routine.arguments == ("t_a(:)", "t_b(:, :)")
...
subroutine driver(some, result)
    ...
    source = Sourcefile.from_source(fcode, frontend=frontend)
    kernel_a = SubroutineItem(
        name="transform_derived_type_arguments_mod#kernel_a", source=source
    )
    kernel = SubroutineItem(
        name="transform_derived_type_arguments_mod#kernel", source=source
    )
    reduce = SubroutineItem(
        name="transform_derived_type_arguments_mod#reduce", source=source
    )
    source_driver = Sourcefile.from_source(
        fcode_driver, frontend=frontend, definitions=source.definitions
    )
    ...
    proc_decls = [
        decl for decl in typedef.declarations if isinstance(decl, ProcedureDeclaration)
    ]
    ...
    source_driver["driver"].apply(
        transformation, role="driver", item=driver, successors=[reduce]
    )
    ...
    assert kernel.routine.arguments == (
        "this_b(:, :)",
        "this_c(:, :)",
        "other_b(:, :)",
        "m",
    ...
    assert reduce.routine.arguments == (
        "start",
        "this_a(:)",
        "this_b(:, :)",
        "this_c(:, :)",
...
subroutine some_routine(t)
    ...
    source2 = Sourcefile.from_source(
        fcode2, frontend=frontend, definitions=source1.definitions
    )
    ...
    transformation = DerivedTypeArgumentsTransformation()
    source1["some_routine"].apply(
        transformation, item=callee, role="kernel", successors=()
    )
    source2["some_routine"].apply(
        transformation, item=caller, role="kernel", successors=(callee,)
    )
...
contains
    ...
    source["caller"].apply(
        transformation, item=caller, role="driver", successors=(callee,)
    )

    assert not caller.trafo_data[transformation._key]
    assert callee.trafo_data[transformation._key]["expansion_map"] == {"t": ("t%arr",)}
...
@pytest.mark.parametrize(
    "frontend", available_frontends(xfail=[(OFP, "No support for recursive prefix")])
)
...
contains
    ...
    source["caller"].apply(
        transformation, item=caller, role="driver", successors=(callee, plus)
    )

    assert not caller.trafo_data[transformation._key]
    assert callee.trafo_data[transformation._key]["expansion_map"] == {"t": ("t%arr",)}
    assert plus.trafo_data[transformation._key]["expansion_map"] == {"t": ("t%arr",)}
...
subroutine caller(t)
    ...
    source_some = Sourcefile.from_source(
        fcode_some, frontend=frontend, definitions=source_header.definitions
    )
    source_other = Sourcefile.from_source(
        fcode_other, frontend=frontend, definitions=source_header.definitions
    )
    source_caller = Sourcefile.from_source(
        fcode_caller,
        frontend=frontend,
        definitions=source_header.definitions
        + source_some.definitions
        + source_other.definitions,
    )
    ...
    transformation = DerivedTypeArgumentsTransformation()
    source_some["sub"].apply(
        transformation, item=some_sub, role="kernel", successors=()
    )
    source_other["sub"].apply(
        transformation, item=other_sub, role="kernel", successors=()
    )
    source_caller["caller"].apply(
        transformation, item=caller, role="driver", successors=(some_sub, other_sub)
    )
...
subroutine driver
    ...
    scheduler = Scheduler(
        ...
        definitions=function_mod.definitions,
        frontend=frontend,
    )

    transformation = TypeboundProcedureCallTransformation(
        duplicate_typebound_kernels=duplicate
    )
    ...
    assert transformation.inline_call_dependencies == {
        "#driver": {
            "typebound_procedure_calls_mod#total_sum",
            "function_mod#some_func",
        },
        "other_typebound_procedure_calls_mod#print_content": {
            "typebound_procedure_calls_mod#total_sum"
        },
    }
    ...
    assert {str(call).lower() for call in calls} == {
        "total_sum(data%stuff(1))",
        "total_sum(data%stuff(2))",
        "some_func(some)",
    }
    ...
    if duplicate:
        ...
        assert [
            str(call.name)
            for call in FindNodes(CallStatement).visit(other_mod["init_"].ir)
