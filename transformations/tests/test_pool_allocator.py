...
def check_stack_created_in_driver(
    ...
    for assignment in assignments:
        if assignment.lhs == "istsz":
            assert str(simplify(assignment.rhs)).lower().replace(" ", "") == str(
                stack_size
            ).lower().replace(" ", "")
            break
    ...
    assert (
        isinstance(assignments[0].rhs, InlineCall)
        and assignments[0].rhs.function == "loc"
    )
    assert "zstack(1, b)" in assignments[0].rhs.parameters
    if generate_driver_stack:
        assert assignments[1].lhs == "ylstack%u" and (
            assignments[1].rhs
            == f"ylstack%l + istsz * c_sizeof(real(1, kind={kind_real}))"
        )
    else:
        assert assignments[1].lhs == "ylstack%u" and (
            assignments[1].rhs
            == f"ylstack%l + c_sizeof(real(1, kind={kind_real}))*istsz"
        )

    # Check that stack assignment happens before kernel call
    assert all(
        loops[0].body.index(a) < loops[0].body.index(first_kernel_call)
...
def test_pool_allocator_temporaries(
    frontend, generate_driver_stack, block_dim, check_bounds, nclv_param
):
    fcode_stack_mod = """
MODULE STACK_MOD
IMPLICIT NONE
...
contains
    ...
    config = {
        "default": {"mode": "idem", "role": "kernel", "expand": True, "strict": True},
        "routine": [
            {
                "name": "driver",
                "role": "driver",
    ...
    if frontend == FP and not generate_driver_stack:
        ...
        Fortran2003.Intrinsic_Name.other_inquiry_names.update(
            {"LOC": {"min": 1, "max": 1}}
        )
        Fortran2003.Intrinsic_Name.generic_function_names.update(
            {"LOC": {"min": 1, "max": 1}}
        )
    ...
    assert (
        kernel_item.trafo_data[transformation._key]["stack_size"] == trafo_data_compare
    )
    assert all(
        v.scope is None
        for v in FindVariables().visit(
    ...
    if generate_driver_stack:
        check_stack_created_in_driver(
            driver, stack_size, calls[0], 1, generate_driver_stack
        )
    else:
        check_stack_created_in_driver(
            driver, stack_size, calls[0], 1, generate_driver_stack, kind_real=kind_real
        )
    ...
    for idx, assign in enumerate(FindNodes(Assignment).visit(kernel.body)):
        if assign.lhs == "ylstack" and assign.rhs == "ydstack":
            ...
            assign.lhs == "ylstack%l"
            and "ylstack%l" in assign.rhs
            and "c_sizeof" in assign.rhs
        ):
            _size = (
                str(assign.rhs)
                .lower()
                .replace(f"*c_sizeof(real(1, kind={kind_real}))", "")
            )
    ...
    for tmp_index in tmp_indices:
        expected_assign_in_order += [
            f"tmp{tmp_index}_ptr_assign",
            f"tmp{tmp_index}_stack_incr",
        ]
...
def test_pool_allocator_temporaries_kernel_sequence(frontend, block_dim, directive):
    if directive == "openmp":
        driver_loop_pragma1 = (
            "!$omp parallel default(shared) private(b) firstprivate(a)\n    !$omp do"
        )
...
contains
    ...
    if frontend == OMNI:
        ...
        parkind_mod = Sourcefile.from_file(
            basedir / "parkind_mod.F90", frontend=frontend
        )
    ...
    transformation = TemporariesPoolAllocatorTransformation(
        block_dim=block_dim, directive=directive, key="some_key"
    )
    scheduler.process(transformation=transformation, reverse=True)
    kernel_item = scheduler["kernel_mod#kernel"]
    ...
    assert (
        kernel_item.trafo_data[transformation._key]["stack_size"]
        == f"c_sizeof(real(1, kind={kind_real}))*klon + c_sizeof(real(1, kind={kind_real}))*klev*klon + 2*c_sizeof(int(1, kind={kind_int}))*klon + c_sizeof(logical(true, kind={kind_log}))*klev"
    )
    # pylint: disable-next=line-too-long
    assert (
        kernel2_item.trafo_data[transformation._key]["stack_size"]
        == f"3*c_sizeof(real(1, kind={kind_real}))*klev*klon"
    )
    assert all(
        v.scope is None
        for v in FindVariables().visit(
    ...
    stack_size += (
        f"3*c_sizeof(real(1, kind={kind_real}))*nz*nlon)/c_sizeof(real(1, kind=jprb))"
    )
    ...
    if directive in ["openmp", "openacc"]:
        ...
        pragmas = [
            p
            for p in FindNodes(Pragma).visit(driver.body)
        ...
        for pragma in pragmas:
            ...
            assert (
                "private" in parameters and "ylstack" in parameters["private"].lower()
            )
    ...
    if directive == ["openacc"]:
        ...
        assert len(pragmas) == 2
        parameters = get_pragma_parameters(
            pragmas[0], starts_with="data", only_loki_pragmas=False
        )
    ...
    for count, item in enumerate([kernel_item, kernel2_item]):
        ...
        for idx, ass in enumerate(FindNodes(Assignment).visit(kernel.body)):
            ...
            if ass.lhs == "ylstack" and ass.rhs == "ydstack":
                ...
                ass.lhs == "ylstack%l"
                and "ylstack%l" in ass.rhs
                and "c_sizeof" in ass.rhs
                and dim1 == _size
            ):
        ...
        for assign1, assign2 in zip(
            expected_assign_in_order, expected_assign_in_order[1:]
        ):
            assert assign_idx[assign2] > assign_idx[assign1]
...
contains
    ...
    assert (
        kernel_item.trafo_data[transformation._key]["stack_size"]
        == f"c_sizeof(real(1, kind={kind_real}))*klon + 4*c_sizeof(real(1, kind={kind_real}))*klev*klon + 2*c_sizeof(int(1, kind={kind_int}))*klon + c_sizeof(logical(true, kind={kind_log}))*klev"
    )
    # pylint: disable-next=line-too-long
    assert (
        kernel2_item.trafo_data[transformation._key]["stack_size"]
        == f"3*c_sizeof(real(1, kind={kind_real}))*columns*levels"
    )
    assert all(
        v.scope is None
        for v in FindVariables().visit(
    ...
    stack_size = (
        f"c_sizeof(real(1, kind={kind_real}))*nlon/c_sizeof(real(1, kind=jwrb)) +"
    )
    stack_size += (
        f"4*c_sizeof(real(1, kind={kind_real}))*nlon*nz/c_sizeof(real(1, kind=jwrb)) +"
    )
    stack_size += (
        f"2*c_sizeof(int(1, kind={kind_int}))*nlon/c_sizeof(real(1, kind=jwrb)) +"
    )
    stack_size += (
        f"c_sizeof(logical(true, kind={kind_log}))*nz/c_sizeof(real(1, kind=jwrb))"
    )
...
def test_pool_allocator_more_call_checks(frontend, block_dim, caplog):
    ...
    scheduler = Scheduler(
        paths=[basedir], config=SchedulerConfig.from_dict(config), frontend=frontend
    )
    if frontend == OMNI:
        for item in scheduler.items:
            normalize_range_indexing(item.routine)
    ...
    if not frontend == OFP:
        ...
        calls = [
            i
            for i in FindInlineCalls().visit(kernel.body)
            if not i.name.lower() in ("c_sizeof", "real")
        ]
        assert len(calls) == 1
        assert calls[0].parameters == ("jl", "ylstack")

    assert (
        "Derived-type vars in Subroutine:: kernel not supported in pool allocator"
        in caplog.text
    )
    rmtree(basedir)
