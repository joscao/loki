...
pytestmark = pytest.mark.skipif(
    "ECWAM_DIR" not in os.environ, reason="ECWAM_DIR not set"
)
...
def fixture_bundle_create(here, local_loki_bundle):
    ...
    execute(["./ecwam-bundle", "create"], cwd=here, silent=False, env=env)
...
def test_ecwam(here, frontend=FP):
    ...
    preprocs = [
        (
            "ecwam-run-preproc",
            "--run-dir=wamrun_48",
            "--config=../source/ecwam/tests/etopo1_oper_an_fc_O48.yml",
        ),
    ...
    if failures:
        msg = "\n".join(
            [f"Non-zero return code {rcode} in {p}" for p, rcode in failures.items()]
        )
        pytest.fail(msg)
