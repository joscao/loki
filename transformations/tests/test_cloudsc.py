...
pytestmark = pytest.mark.skipif(
    "CLOUDSC_DIR" not in os.environ, reason="CLOUDSC_DIR not set"
)
...
def fixture_bundle_create(here, local_loki_bundle):
    ...
    execute(["./cloudsc-bundle", "create"], cwd=here, silent=False, env=env)
...
@pytest.mark.parametrize(
    "frontend",
    available_frontends(
        skip=[(OMNI, "OMNI needs FParser for parsing dependencies")]
        if not HAVE_FP
        else None
    ),
)
def test_cloudsc(here, frontend):
    ...
    for binary, *args in binaries:
        ...
        try:
            ...
            results = pd.read_fwf(
                io.StringIO(output.stdout.decode()), index_col="Variable"
            )
